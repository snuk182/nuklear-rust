 ...
 * ===============================================================*/
int
nk_filter_cyrillic(const struct nk_text_edit *box, nk_rune c)
{
    NK_UNUSED(box);
    if ((c > 0x0400 && c < 0x052F) || (c > 0x2DE0 && c < 0x2DFF) || (c > 0xA640 && c < 0xA69F)) return nk_true;
...
grid_demo(struct nk_context *ctx, struct media *media)
{
        ...
        NK_WINDOW_NO_SCROLLBAR))
    {
        ...
        nk_label(ctx, "Cyrillic:", NK_TEXT_RIGHT);
		nk_edit_string(ctx, NK_EDIT_FIELD, text[3], &text_len[3], 64, NULL);
